...
void Indexer::addTerm(string term, Pair p){
    ...
    if (Index.contains(firstletter)) {  // bas harf var
         if (Index.getVal(firstletter).getTree().contains(term)) {
            Index.getVal(firstletter).getTree().getVal(term).addPage(p);
         }
         else {
            TermTreeNodeData newtermnode(term, p);
            Index.getVal(firstletter).getTree().add(newtermnode);
         } 
    }
    else {                            // bas harf yok
        BinarySearchTree<TermTreeNodeData> newtree;
        TermTreeNodeData  newtermnode(term, p);
        newtree.add(newtermnode);
        IndexNodeData newnode(firstletter, newtree);
...
void Indexer::removeTerm(string term){
    ...
    Index.getVal(firstletter).getTree().remove(term);

    if (Index.getVal(firstletter).getTree().height() == -1) {
        Index.remove(firstletter);
    }
...
void Indexer::makeLetterBalanced(char letter){
    // TODO
    // Write your code here
    Index.getVal(letter).getTree().makeItBalanced();
